# 🌤️ 鸿蒙天气应用 WeatherAPP

基于 HarmonyOS NEXT 开发的智能天气查询应用，提供实时天气、逐日预报、城市搜索、专业气象服务入口等丰富功能。采用科幻深海风格 UI，支持 34 个主要城市快捷切换，并集成中国气象局、Windy 等专业平台。


---

## 📱 功能特性

### 1. 实时天气查询
- 输入任意城市名（支持中文），获取**当前温度、天气状况、湿度、风向风力**
- 数据来源：高德地图天气 API（实时更新）
- 智能匹配城市 POI，支持模糊搜索

![实时天气](screenshots/广州.jpg)

### 2. 未来两天预报
- 展示**今天 + 明天**的天气概况
- 自动区分白天/夜间：根据当前时间智能显示日间或夜间温度
- 包含天气图标、温度、天气文字描述

![未来预报](screenshots/上海.jpg)

## 📱 功能特性

### 1. 实时天气查询
输入任意城市名（支持中文），获取**当前温度、天气状况、湿度、风向风力**
数据来源：高德地图天气 API（实时更新）
智能匹配城市 ，支持模糊搜索




### 2. 未来两天预报
展示**今天 + 明天**的天气概况
自动区分白天/夜间：根据当前时间智能显示日间或夜间温度
包含天气图标、温度、天气文字描述


### 3. 城市快捷入口
- 提供 **34 个直辖市、省会及特别行政区** 按钮
- 采用网格布局，一触即达
- 每个城市配有特色 emoji（如北京🏯、成都🐼）


![城市快捷入口](screenshots/成都.jpg)


### 4. 智能城市搜索
- 点击搜索按钮弹出对话框
- 支持**模糊搜索**：输入“广州”可匹配“广州市”、“广东省广州市”等
- 搜索结果来自高德 POI 接口，自动过滤出城市/区县级别结果
- 点击结果直接切换城市并刷新天气


![城市搜索](screenshots/搜索功能.jpg)




### 5. 专业气象服务集成
内置多个权威气象平台入口，一键跳转：

| 服务 | 说明 | 来源 |

| 🛰️ 卫星云图 | 风云二号红外云图，实时动态 | 中央气象台 |
| 🇨🇳 时局图 | 中国天气网气象云图产品 | 国家气象局 |
| 🌍 Ventusky | 全球风场、气温、气压可视化 | 捷克公司 |
| 💨 Windy | 高分辨率气象动画，支持多图层 | 国际平台 |
| 🌐 百度天气 | 综合搜索当前城市详细预报 | 百度搜索 |
HEAD
- 点击以下按钮可切换到专业气象服务界面
![专业气象服务](screenshots/展示专业服务.jpg)
- 以下是卫星云图界面
![专业气象服务](screenshots/卫星云图.jpg)
- 以下是中央气象台界面
![专业气象服务](screenshots/中央气象台.jpg)
- 以下是"Ventusky"专业气象软件界面
![专业气象服务](screenshots/Ventusky1.jpg)
![专业气象服务](screenshots/Ventusky2.jpg)
- 以下是"Windy"全球风场图
![专业气象服务](screenshots/Windy1.jpg)
![专业气象服务](screenshots/Windy2.jpg)



### 6. 科幻深海 UI 设计
- 深蓝渐变背景 + 毛玻璃卡片效果
- 动态天气 emoji 实时映射（晴☀️、雨🌧️、雪❄️等）
- 城市特色 emoji，增强辨识度
- 阴影与圆角营造舒适视觉

### 7. 全平台适配
- 支持手机、折叠屏、平板（响应式布局）
- 滚动视图确保所有内容可触达


---

## 📁 项目目录结构



---

## 设计思路

### 为什么选择科幻深海风格？
- **沉浸感**：深蓝渐变模拟从深海到星空的过渡
- **信息清晰**：白色文字与毛玻璃卡片对比舒适
- **情感关联**：天气 emoji 直观传递天气情绪


### 交互细节
- **搜索弹窗**：点击外部关闭，符合移动端习惯
- **快捷按钮**：34个城市网格布局，一目了然
- **预测展示**：未来两天卡片并列，方便对比
- - **便捷性**： 省会城市快捷按钮方便随时点击查询

---

## 技术栈

## 📁 项目目录结构
WeatherAPP/
│
├── AppScope/ # 应用全局配置
│ ├── resources/ # 应用级资源
│ │ └── base/
│ │ ├── element/ # 字符串、颜色常量
│ │ └── media/ # 应用图标
│ └── app.json5 # 应用唯一标识、版本号
│
├── entry/ # 主模块
│ └── src/
│ ├── main/
│ │ ├── ets/
│ │ │ ├── entryability/
│ │ │ │ └── EntryAbility.ets # 应用启动入口
│ │ │ └── pages/
│ │ │ ├── Index.ets # 🌤️ 核心页面（所有功能）
│ │ │ └── WebViewPage.ets # 内嵌浏览器页面
│ │ ├── resources/
│ │ │ ├── base/
│ │ │ │ ├── element/ # 字符串常量(string.json)
│ │ │ │ ├── media/ # 天气图标（sunny.png等）
│ │ │ │ └── profile/ # 路由表配置
│ │ │ └── rawfile/ # 原始文件（预留）
│ │ └── module.json5 # ⚙️ 权限声明、Ability注册
│ └── ohosTest/ # 单元测试目录
│
├── hvigor/ # 构建脚本目录
│ └── hvigor-config.json5 # 构建优先级配置
│
├── oh_modules/ # 第三方依赖包（自动生成）
│
├── screenshots/ # 📸 应用截图文件夹
│ ├── 广州.jpg # 实时天气截图
│ ├── 上海.jpg # 未来预报截图
│ ├── 成都.jpg # 城市快捷入口截图
│ └── 搜索功能.jpg # 城市搜索截图
│
├── .gitignore # 🚫 Git忽略配置
├── build-profile.json5 # 🔧 签名配置、产品类型
├── hvigorfile.ts # 项目级构建脚本
├── oh-package.json5 # 📦 项目依赖管理
├── oh-package-lock.json5 # 依赖锁定文件
└── README.md # 📖 项目说明文档

### 重点文件说明

| 文件/目录 | 作用 | 是否提交Git |
| :--- | :--- | :--- |
| `Index.ets` | 所有核心功能代码：天气请求、UI渲染、城市切换、搜索逻辑 | ✅ 是 |
| `module.json5` | 声明 `ohos.permission.INTERNET` 网络权限 | ✅ 是 |
| `local.properties` | 本地 SDK 路径（每个人电脑不同） | ❌ 否 |
| `.temp/`、`build/` | 编译临时文件，占用空间大 | ❌ 否 |
| `resources/base/media/` | 存放天气图标（如 `sunny.png`） | ✅ 是 |
| `WebViewPage.ets` | 打开卫星云图等外部网页的容器 | ✅ 是 |

---

## 🎨 设计思路

### 为什么选择科幻深海风格？
- **沉浸感**：深蓝渐变模拟从深海到星空的过渡，让用户感觉正在探索未知气象世界
- **信息清晰**：白色文字与半透明毛玻璃卡片在深色背景上阅读舒适，对比度适中
- **情感关联**：天气 emoji（☀️🌧️⛄）比文字更直观，能快速传递天气情绪

### 交互细节
- **搜索弹窗**：半透明背景遮罩，点击外部即可关闭，符合主流移动端习惯
- **快捷按钮**：34 个城市采用网格布局，减少滑动成本，高频城市一目了然
- **预测展示**：未来两天采用卡片并列，方便对比昼夜温差

---

## 🛠️ 技术栈


| 类别 | 技术 |
| :--- | :--- |
| 开发环境 | DevEco Studio NEXT Beta1+ |
<<<<<<< HEAD
| 开发语言 | ArkTS |
| UI 框架 | HarmonyOS SDK (API 9+) |
| 网络请求 | `@ohos.net.http` |
| 天气 API | 高德地图天气 API v3 |

---

## 快速开始

### 环境要求
- DevEco Studio 5.0.3+
- HarmonyOS SDK API 9+
=======
| 开发语言 | ArkTS (TypeScript 超集) |
| UI 框架 | HarmonyOS SDK (API 9+) |
| 网络请求 | `@ohos.net.http` |
| 路由跳转 | `@ohos.router` + `@ohos.ability.common` |
| 外部链接 | `openLink` API |
| 天气 API | 高德地图天气 API v3 (Web服务) |
| 图标方案 | Unicode Emoji + 自定义符号 |
| 布局 | Flex、Grid、Scroll、Stack |

---

## 🚀 快速开始

### 环境要求
- Windows 10/11 或 macOS 12+
- DevEco Studio 5.0.3 或更高版本
- HarmonyOS SDK API 9 及以上
- 真机：HarmonyOS 2.0+ 或 荣耀 Magic UI 4.0+
- 模拟器：需配置好网络（无代理）


### 安装与运行

1. **克隆仓库**
   ```bash
<<<<<<< HEAD
   git clone https://github.com/OSSD-Course-SYSU-2/WeatherAPP25307076.git
=======
   git clone https://github.com/OSSD-Course-SYSU-2/WeatherAPP25307076.git

